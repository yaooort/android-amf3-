...
import com.weedong.flex.messaging.amf.io.AMF0Deserializer;
import com.weedong.flex.messaging.amf.io.AMF0Serializer;
...
public class AMFConnection
{
     ...
     */
    protected List<AMF0Header> amfHeaders;
    ...
    {
        if (amfHeaders == null)
            amfHeaders = new ArrayList<AMF0Header>();

        AMF0Header header = new AMF0Header(name, mustUnderstand, data);
    ...
    public boolean removeAmfHeader(String name)
    {
        ...
        if (amfHeaders != null)
        {
            for (Iterator<AMF0Header> iterator = amfHeaders.iterator(); iterator.hasNext();)
            {
            	AMF0Header header = iterator.next();
    ...
    public Object call(String command, Object ... arguments) throws ClientStatusException , ServerStatusException
    {
        ...
        AMF0Message requestMessage = new AMF0Message();

        if (amfHeaders != null)
        {
            for (AMF0Header header : amfHeaders)
                requestMessage.addHeader(header);
        }

        AMF0Body amfMessage = new AMF0Body(command, responseURI, arguments);
        requestMessage.addBody(amfMessage);

        // Setup for AMF message serializer
        actionContext.setRequestMessage(requestMessage);
        ByteArrayOutputStream outBuffer = new ByteArrayOutputStream();
        AMF0Serializer amfMessageSerializer = new AMF0Serializer(new DataOutputStream(outBuffer));
            ...
            ServerStatusException
    {
        ...
        AMF0Message message = null;
        actionContext.setRequestMessage(message);
        AMF0Deserializer deserializer = new AMF0Deserializer(new DataInputStream(urlConnectionInputStream));
        try
        {
            message = deserializer.getAMFMessage();
        }
        catch (MessageException me)
        {
            // Means the stream contained non-AMF data, reset the stream and throw.
            if (AMF0Deserializer.CODE_VERSION_MISMATCH.equals(me.getCode()))
            {
     ...
     */
    protected Object processAmfPacket(AMF0Message packet) throws ServerStatusException
    {
     ...
     */
    protected Object processAmfBody(Iterator<AMF0Body> messages) throws ServerStatusException
    {
        for (Iterator<AMF0Body> iter = messages; iter.hasNext();)
        {
        	AMF0Body message = iter.next();
            String targetURI = message.getTarget();
            ...
            else if (targetURI.endsWith(MessageIOConstants.STATUS_METHOD))
            {
                ...
                ServerStatusException exception = new ServerStatusException(exMessage, message.getValue(), responseInfo);
                throw exception;
            }
        }
    ...
    protected void setHttpRequestHeaders()
    {
        ...
        urlConnection.setRequestProperty(HTTP_HEADER_NAME_CONTENT_TYPE, MessageIOConstants.AMF_CONTENT_TYPE);
    }
